...
As per NAMI(National Alliance on Mental Illness, 21% of U.S. adults experienced mental illness in 2020 (52.9 million people). This represents 1 in 5 adults. Mental Health related issues can lead to substance abuse and potentially harm yourself or others. 
...
The repo is structured such that the entire code required to excecute the project is under the MIMO folder. The project is developed on a Windows machine so Ubuntu or Mac users may face some installation issues but its highly unlikely.
...
4. Now before you set up Rasa, you need to first make sure that you are able to connect with our MySQL database. Currently the project is hosted on AWS RDS and you need to place the connections.py share in the code/actions folder to ensure connection is established. The file is sent seperately via mail and is also available in Project submission but not shared in the repo for security reasons.
5. Instead of step 4, One can also create their own local database by using the [Readme.md](https://github.com/dheeralbhole/MIMO/tree/main/mysql_details) file shared here for steps.
6. Now you can start rasa in local mode by first enabling the action server by using the command: rasa run actions
7. Then you can start Rasa-X interactive GUI by running the command in another terminal in the same environment: rasa x

If you face any installation issues please feel free to contact me at dheeral.bhole@mavs.uta.edu
...
## Known Exisiting Issues 

- Even after ample Naming examples in the NLU file, while entrying names the user has to be a little bit verbose. Like instead of just the name "Dheeral Bhole", its more apt to use "My name is Dheeral Bhole"
